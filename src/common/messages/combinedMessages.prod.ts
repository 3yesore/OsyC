export const allMessages: Readonly<Record<string, Readonly<Record<string, string>>>> = {
    ...
    "Doctor.Dialogue.Main": {
        ...
        zh: "您好！配置医生因 ${activateReason} 而启动！\n遗憾的是，部分配置被检测出存在潜在问题。\n请放心，我们会逐一解决这些问题。\n\n先提前告知您，我们将就以下项目进行确认：\n\n${issues}\n\n我们开始处理吗？",
        "zh-tw":
    ...
    "Setting.GenerateKeyPair.Desc": {
        ...
        ru: 'Мы сгенерировали пару ключей!\n\nПримечание: эта пара ключей больше не будет показана. Сохраните её в надёжном месте. Если вы её потеряете, потребуется создать новую пару ключей.\nПримечание 2: открытый ключ имеет формат spki, закрытый ключ — формат pkcs8. Для удобства копирования переводы строк в открытом ключе преобразуются в `\\n`.\nПримечание 3: открытый ключ следует настроить в удалённой базе данных, а закрытый ключ — на локальных устройствах.\n\n>[!FOR YOUR EYES ONLY]-\n> <div class="sls-keypair">\n>\n> ### Открытый ключ\n> ```\n${public_key}\n> ```\n>\n> ### Закрытый ключ\n> ```\n${private_key}\n> ```\n>\n> </div>\n\n>[!Both for copying]-\n>\n> <div class="sls-keypair">\n>\n> ```\n${public_key}\n${private_key}\n> ```\n>\n> </div>\n',
    ...
    "Setup.QRCode": {
        ...
        ru: 'Мы сгенерировали QR-код для передачи настроек. Отсканируйте QR-код телефоном или другим устройством.\nПримечание: QR-код не зашифрован, поэтому открывайте его с осторожностью.\n\n>[!FOR YOUR EYES ONLY]-\n> <div class="sls-qr">${qr_image}</div>',
