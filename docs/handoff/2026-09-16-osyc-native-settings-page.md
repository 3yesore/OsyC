...
- `2.0.6` 无 tag、无 Release。本轮改动**已提交**到 `codex/2.0.6-stabilize`（`3c9765d` 设置页、`756327d` 指纹刷新、其后一条文档提交），**尚未推送**；领先 origin 的提交数以 `git status -sb` 为准。

## 待办

1. **真机验收（未完成）**：iOS/Android 上确认四分组布局、折叠项展开、账户条点击路径。
2. **推送（未完成）**：本轮提交已在本地，`git push` 尚未执行。执行前请先读第 6 条。
