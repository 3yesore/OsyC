...
const themePackFiles = {
    minimal: "./assets/themes/minimal/original.css",
    chineseWriting: "./assets/themes/chinese-writing/original.css",
};

function scopeThemeCssForBundle(css) {
    const root = postcss.parse(css);
    root.walkAtRules("import", (rule) => rule.remove());
    root.walkRules((rule) => {
        if (rule.parent?.type === "atrule" && /keyframes/i.test(rule.parent.name)) return;
        try {
            const process = selectorParser((selectors) => {
                selectors.each((selector) => {
                    const first = selector.nodes.find((node) => node.type !== "comment");
                    const replacesRoot = first && ((first.type === "tag" && (first.value === "body" || first.value === "html")) || (first.type === "pseudo" && first.value === ":root"));
                    if (replacesRoot) {
                        selector.removeChild(first);
                        selector.prepend(selectorParser.className({ value: "osyc-theme-notes" }));
                    } else {
            ...
            rule.selector = process.processSync(rule.selector);
        } catch {
            // An unparseable selector is safer to omit from the isolated notes variant.
            rule.remove();
        }
    });
...
const themePackDefines = Object.fromEntries(Object.entries(themePackFiles).flatMap(([name, file]) => {
    const css = fs.readFileSync(file, "utf8");
    const key = name === "minimal" ? "MINIMAL" : "CHINESE_WRITING";
    return [
        [`__OSYC_THEME_${key}_CSS__`, JSON.stringify(css)],
        [`__OSYC_THEME_${key}_NOTES_CSS__`, JSON.stringify(scopeThemeCssForBundle(css))],
    ];
}));
